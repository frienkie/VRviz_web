...
VRViz 的静态产品说明页，可直接部署到 GitHub Pages。VRViz 是在 Meta Quest 本地运行的一体机应用，不依赖 PC 连接、PC 串流或数据线；仅需通过网络访问 ROS/rosbridge 数据源。页面还涵盖手柄双模式、Audius/Spotify 在线音乐、摄像头图像、STL 模型导入、数据流程、快速开始和三语隐私政策。
...
- TurtleBot3 与 Quest 3 模型轮廓图：`assets/*-model-outline.png`
- 模型轮廓再生成工具：`scripts/render_model_icons.py`
