...
Object.assign(translations.zh, {
  rosGuideKicker: "ROSBRIDGE / 连接准备",
  rosGuideTitle: "为 ROS 1 与 ROS 2 准备 WebSocket",
  rosGuideLead: "在运行 ROS 的电脑上安装并启动 rosbridge，再将同一网络内可访问的 IP 与端口填入 VRViz。",
  rosInstallTitle: "安装 rosbridge",
  ros1InstallText: "在 ROS 电脑上执行：",
  ros2InstallText: "更新软件源，然后只安装与你的 ROS 2 发行版对应的包：",
  rosLaunchTitle: "启动 WebSocket",
  ros1LaunchText: "加载 Noetic 环境并启动 rosbridge：",
  ros2LaunchText: "加载对应的 ROS 2 环境，再使用当前官方 XML 启动文件：",
  rosCompatibilityTitle: "版本兼容",
  rosCompatibilityText: "如果提示找不到启动文件，可先查看本机 rosbridge 包实际安装的文件：",
  rosConnectionIndex: "03 / 连接 VRVIZ",
  rosConnectionTitle: "填写 ROS 电脑的 IP 与端口",
  rosConnectionText: "rosbridge 默认使用 9090 端口。请把示例 IP 替换为运行 ROS 的电脑在局域网中的 IPv4 地址。",
  rosEndpointTemplate: "ws://<ROS 电脑 IP>:9090",
  rosAddressExampleLabel: "完整地址示例",
  rosIpFieldLabel: "VRViz · IP",
  rosPortFieldLabel: "VRViz · Port",
  rosNetworkNoteTitle: "连接检查",
  rosNetworkNoteText: "Quest 与 ROS 电脑需处于可互通的网络，并允许 TCP 9090 通过防火墙。IP 栏只填地址，不要填 ws://，也不要使用 localhost 或 127.0.0.1；请仅在可信局域网或 VPN 中开放 rosbridge。",
});

Object.assign(translations.ja, {
  rosGuideKicker: "ROSBRIDGE / 接続準備",
  rosGuideTitle: "ROS 1 / ROS 2 のWebSocketを準備",
  rosGuideLead: "ROSを実行しているPCにrosbridgeをインストールして起動し、同じネットワークから到達できるIPアドレスとポートをVRVizへ入力します。",
  rosInstallTitle: "rosbridgeをインストール",
  ros1InstallText: "ROS PCで次を実行します。",
  ros2InstallText: "パッケージ情報を更新し、使用中のROS 2ディストリビューションに対応するパッケージだけをインストールします。",
  rosLaunchTitle: "WebSocketを起動",
  ros1LaunchText: "Noetic環境を読み込み、rosbridgeを起動します。",
  ros2LaunchText: "使用中のROS 2環境を読み込み、現在の公式XML起動ファイルを使用します。",
  rosCompatibilityTitle: "バージョン互換",
  rosCompatibilityText: "起動ファイルが見つからない場合は、インストール済みrosbridgeパッケージのファイルを確認してください。",
  rosConnectionIndex: "03 / VRVIZに接続",
  rosConnectionTitle: "ROS PCのIPアドレスとポートを入力",
  rosConnectionText: "rosbridgeは既定でポート9090を使用します。サンプルIPを、ROSを実行しているPCのLAN内IPv4アドレスに置き換えてください。",
  rosEndpointTemplate: "ws://<ROS PCのIP>:9090",
  rosAddressExampleLabel: "完全なアドレス例",
  rosIpFieldLabel: "VRViz · IP",
  rosPortFieldLabel: "VRViz · Port",
  rosNetworkNoteTitle: "接続チェック",
  rosNetworkNoteText: "QuestとROS PCを互いに到達できるネットワークへ接続し、ファイアウォールでTCP 9090を許可してください。IP欄にはアドレスだけを入力し、ws://は付けず、localhostや127.0.0.1も使用しないでください。rosbridgeは信頼できるLANまたはVPN内でのみ公開してください。",
});

Object.assign(translations.en, {
  rosGuideKicker: "ROSBRIDGE / CONNECTION SETUP",
  rosGuideTitle: "Prepare WebSocket for ROS 1 or ROS 2",
  rosGuideLead: "Install and start rosbridge on the computer running ROS, then enter an IP address and port that Quest can reach on the same network.",
  rosInstallTitle: "Install rosbridge",
  ros1InstallText: "Run these commands on the ROS computer:",
  ros2InstallText: "Update the package list, then install only the package that matches your ROS 2 distribution:",
  rosLaunchTitle: "Start WebSocket",
  ros1LaunchText: "Load the Noetic environment and start rosbridge:",
  ros2LaunchText: "Load the matching ROS 2 environment, then use the current official XML launch file:",
  rosCompatibilityTitle: "Version compatibility",
  rosCompatibilityText: "If the launch file is not found, list the files installed by your local rosbridge package:",
  rosConnectionIndex: "03 / CONNECT VRVIZ",
  rosConnectionTitle: "Enter the ROS computer's IP and port",
  rosConnectionText: "rosbridge uses port 9090 by default. Replace the example IP with the LAN IPv4 address of the computer running ROS.",
  rosEndpointTemplate: "ws://<ROS computer IP>:9090",
  rosAddressExampleLabel: "Full address example",
  rosIpFieldLabel: "VRViz · IP",
  rosPortFieldLabel: "VRViz · Port",
  rosNetworkNoteTitle: "Connection check",
  rosNetworkNoteText: "Quest and the ROS computer must be on mutually reachable networks, and the firewall must allow TCP port 9090. Enter only the address in the IP field—do not include ws://, localhost, or 127.0.0.1. Expose rosbridge only on a trusted LAN or VPN.",
});
